...
export default function SourceFilesPage() {
    ...
    const loadFiles = useCallback(async () => {
        setLoading(true);
        try {
            const res = await api.get('/transformation/files/?type=source');
            setFiles(res.data);
        } catch {
            // silent
        } finally {
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                <Box>
                    ...
                    <Typography variant="body2" color="text.secondary" sx={{ mt: 0.25 }}>
                        All uploaded source files with extracted column headers
                    </Typography>
                </Box>
                <Chip
                ...
                {filtered.length === 0 && !loading && (
                    <Box sx={{ p: 4, textAlign: 'center' }}>
                        ...
                        <Typography variant="body2" color="text.secondary">
                            {search ? 'No matching files found' : 'No source files uploaded yet'}
                        </Typography>
                    </Box>
                )}
